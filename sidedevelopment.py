...
def llama321Binstruct_generate_response(prompt):
    ...
    llama321Binstruct_model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-1B-Instruct",torch_dtype=torch.float16)
...
def llama323b_generate_response(prompt):
    ...
    llama323Bmodel = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-3B", torch_dtype=torch.float16).to(
        'cuda')
...
def gpt2_generate_response(prompt):
...
def generate_response(prompt):
    ...
    elif intent == "complex_text_generation":
        return llama323binstruct_generate_response(prompt)
    elif intent == "simple_text_generation":
        return llama323b_generate_response(prompt)
    else:
        return gpt2_generate_response(prompt)
